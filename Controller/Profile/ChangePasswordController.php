...
/**
 * Profile change password controller
 */
class ChangePasswordController
{
    ...
    {
        $form    = $this->formFactory->createPasswordChangeForm()->handleRequest($request);
        $partial = $request->isXmlHttpRequest();

        if (!$form->isSubmitted()) {
            return new Response($this->formRenderer->renderPasswordChangeForm($form, $partial));
        }

        $successMessage = 'profile.change_password.success';

        if (!$this->formHandler->handlePasswordChangeForm($form, !$partial, $successMessage)) {
            $html = $this->formRenderer->renderPasswordChangeForm($form, $partial);
        ...
        if ($partial) {
            return new AjaxResponse($this->formRenderer->renderPasswordChangeForm($form), true, $successMessage);
        }

        return new RedirectResponse($this->router->generate('darvin_user_profile_change_password'));
    }
}
